...
if args.load_model:
    ...
    model = vxm.networks.VxmDense_joint_level1.load(args.load_model, device)
else:
#     # otherwise configure new model
    model = vxm.networks.VxmDense_joint_level1(
        inshape=inshape,
...
torch.save(netG_A2B.state_dict(), 'model_joint_R_G_level1/netG_A2B.pth')
torch.save(netG_B2A.state_dict(), 'model_joint_R_G_level1/netG_B2A.pth')
torch.save(netD_A.state_dict(), 'model_joint_R_G_level1/netD_A.pth')
torch.save(netD_B.state_dict(), 'model_joint_R_G_level1/netD_B.pth')
...
def every_epoch_val(best_dice):
    ...
    if best_dice < accurent_dice:
        ...
        model.save(os.path.join('model_joint_R_G_level1', '%04d_best.pt' % epoch))
        # save cyclegan
        torch.save(netG_A2B.state_dict(), 'model_joint_R_G_level1/best_netG_A2B.pth')
        torch.save(netG_B2A.state_dict(), 'model_joint_R_G_level1/best_netG_B2A.pth')
        torch.save(netD_A.state_dict(), 'model_joint_R_G_level1/best_netD_A.pth')
        torch.save(netD_B.state_dict(), 'model_joint_R_G_level1/best_netD_B.pth')
