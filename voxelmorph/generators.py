...
def volgen(
    ...
    if isinstance(vol_names_1, str):
        if os.path.isdir(vol_names_1):
            vol_names_1 = os.path.join(vol_names_1, '*')
        vol_names_1 = glob.glob(vol_names_1)
    
    if isinstance(vol_names_2, str):
        if os.path.isdir(vol_names_2):
            vol_names_2 = os.path.join(vol_names_2, '*')
        vol_names_2 = glob.glob(vol_names_2)
        

    # if isinstance(segs, list) and len(segs) != len(vol_names):
    #     raise ValueError('Number of image files must match number of seg files.')
    # print('vol_names_1',vol_names_1)
    # print('vol_names_2',vol_names_2)
    # print('total pairs:',len(vol_names_1))
    # print('total pairs:',len(vol_names_2))
    # print('total pairs:',len(vol_names_3))
    # print('total pairs:',len(vol_names_4))

    # print('vol_names',vol_names)
    while True:
        # generate [batchsize] random image indices
        indices = np.random.randint(len(vol_names_1), size=batch_size)
        ...
        imgs_1 = [py.utils.load_volfile_normalization(vol_names_1[i], **load_params) for i in indices]
        
        
        imgs_2 = [py.utils.load_volfile_normalization(vol_names_2[i], **load_params) for i in indices]
        
        # labels_1 = [py.utils.load_volfile_normalization(vol_names_3[i], **load_params) for i in indices]
        # labels_2 = [py.utils.load_volfile_normalization(vol_names_4[i], **load_params) for i in indices]
       
        vols = [np.concatenate(imgs_1, axis=0),np.concatenate(imgs_2, axis=0)]
        # vols = [np.concatenate(imgs_1, axis=0),np.concatenate(imgs_2, axis=0),np.concatenate(labels_1, axis=0),np.concatenate(labels_2, axis=0)]
...
def volgen_iseg2017(
    ...
    while True:
        # generate [batchsize] random image indices
        indices_1 = np.random.randint(len(vol_names_1), size=batch_size)
        
        # only batchsize == 1
        while True:
            indices_2 = np.random.randint(len(vol_names_2), size=batch_size)
        ...
        # print('vol_names_2',vol_names_2)
        # print('indices',indices)
        imgs_1 = [py.utils.load_volfile_normalization(vol_names_1[i], **load_params) for i in indices_1]
        
        
        imgs_2 = [py.utils.load_volfile_normalization(vol_names_2[i], **load_params) for i in indices_2]
...
def volgen_iseg2017_slices(
    ...
    # note that vol_names is a dictionary
    ...
    while True:
        ...
        random_key = random.choice(list(vol_names_1.keys()))
        # generate [batchsize] random image indices
        # print(len(vol_names_1[random_key]))
        indices_1 = np.random.randint(len(vol_names_1[random_key]), size=batch_size)
        
        # only batchsize == 1
        while True:
            indices_2 = np.random.randint(len(vol_names_2[random_key]), size=batch_size)
            if indices_1 != indices_2:
                break
        ...
        # print('indices',indices)
        # print('OK')
        imgs_1 = [py.utils.load_volfile_normalization(vol_names_1[random_key][i], **load_params) for i in indices_1]
         
        imgs_2 = [py.utils.load_volfile_normalization(vol_names_2[random_key][i], **load_params) for i in indices_2]
        # print('OK')
...
def scan_to_scan(vol_names_1,vol_names_2, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    """
    ...
    """
    zeros = None
    gen = volgen(vol_names_1,vol_names_2, batch_size=batch_size, **kwargs)
    
    # i = 0
    while True:
        scan = next(gen)
        scan1 = scan[0]
        # print('typelen',len(next(gen)))
        scan2 = scan[1]
        # i = i+1
        # print('scan1.type',type(scan1))
        # print(i)
        ...
        # print('invols.shape',invols.shape)
        # print('outvols.shape',outvols.shape)
        # print('len(invols)',len(invols))
        # print('len(outvols)',len(outvols))

        yield (invols, outvols)

def scan_to_scan_with_label(vol_names_1,vol_names_2,vol_names_3,vol_names_4, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    """
    Generator for scan-to-scan registration.
    ...
    """
    zeros = None
    gen = volgen(vol_names_1,vol_names_2,vol_names_3,vol_names_4, batch_size=batch_size, **kwargs)
    ...
    while True:
        ...
        # print('len(invols)',len(invols))
        # print('len(outvols)',len(outvols))
        outvols.append(scan4)

        
        yield (invols, outvols)  

def scan_to_scan_iseg2017(vol_names_1,vol_names_2, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    """
    Generator for scan-to-scan registration.
    ...
    """
    zeros = None
    gen = volgen_iseg2017(vol_names_1,vol_names_2, batch_size=batch_size, **kwargs)
...
def scan_to_scan_iseg2017_slices(vol_names_1,vol_names_2, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    """
    note that volnames is a dictionary
    """
    zeros = None
    gen = volgen_iseg2017_slices(vol_names_1,vol_names_2, batch_size=batch_size, **kwargs)
